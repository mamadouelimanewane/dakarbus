...
export default function DriverApp() {
  ...
  useEffect(() => {
    ...
    const id = navigator.geolocation.watchPosition(
      p => { const l:[number,number]=[p.coords.latitude,p.coords.longitude]; setLocation(l); dispatch(setUserLocation(l));
        const s = p.coords.speed ?? 0; setSpeed(s > 0 ? Math.round(s*3.6) : Math.floor(25+Math.random()*30)); },
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video:{ facingMode:'environment' } })
      .then(s => {
        stream = s;
        const vid = videoRef.current!; vid.srcObject = s; vid.setAttribute('playsinline','true'); vid.play();
        const tick = () => {
      ...
      .catch(e => {
        const msg = e.name==='NotAllowedError' ? 'Accès caméra refusé. Vérifiez les permissions.'
  ...
  const sendIncident = () => {
    ...
    if (!d) return;
    dispatch(addReport({ id:Math.random().toString(36).substring(2,11), type:incidentType,
      description:`[${name}] ${d}`, location:location??[14.7167,-17.4677], timestamp:Date.now(), upvotes:0 }));
    dispatch(showToast({ type:'success', message:'Incident signalé avec succès !' }));
      ...
      <header className="flex-shrink-0 flex items-center justify-between px-4 h-14"
        ...
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg"
            style={{ background:`linear-gradient(135deg,${lineColor}88,${lineColor}55)`, border:`1px solid ${lineColor}44` }}>
            👨‍✈️
          </div>
          <div>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-3 pb-safe">
        ...
        <div className="rounded-3xl p-5 relative overflow-hidden transition-all duration-500"
          style={{
            ...
            boxShadow: status==='driving' ? '0 16px 48px rgba(5,150,105,.1)' : 'none',
          }}>

          {/* Glow */}
          {status==='driving' && (
            <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full opacity-15 pointer-events-none"
              style={{ background:'radial-gradient(circle, #059669, transparent)', filter:'blur(30px)' }} />
          )}
          ...
          <div className="relative">
            ...
            <div className="flex justify-center mb-4">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl relative"
                style={{ background:status==='driving'?'rgba(5,150,105,.2)':'rgba(255,255,255,.05)', border:`1px solid ${status==='driving'?'rgba(5,150,105,.3)':'var(--c-border)'}` }}>
                {status==='driving' ? '🚌' : '☕'}
                {status==='driving' && (
                  <div className="absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-current"
                    style={{ background:'#22c55e', borderColor:'var(--c-bg)' }}>
                    <div className="absolute inset-0 rounded-full bg-emerald-400" style={{ animation:'pulse-ring 2s ease-out infinite' }} />
                  </div>
                )}
              </div>
            </div>
            ...
            <h2 className="text-center text-lg font-black text-white mb-1">
              {status==='driving' ? 'En service' : 'Hors service'}
            </h2>
            ...
            {status==='driving' && location && (
              <div className="flex items-center justify-center gap-2 mb-3 text-[10px] font-mono"
            ...
            <button
              onClick={() => { setStatus(s=>s==='driving'?'idle':'driving'); if(status==='driving'){setShiftSecs(0);setPassengers(0);} }}
        ...
        {status==='driving' && (
          <div className="grid grid-cols-2 gap-3">
            {/* Scan */}
            <button
              onClick={() => { setScanMode(true); setScanResult(null); setCameraError(null); }}
            ...
            <div className="flex flex-col items-center gap-2.5 py-3 px-3 rounded-2xl card">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setPassengers(n=>n+1)}
                  className="w-8 h-8 rounded-xl font-black text-lg transition-all active:scale-90"
            ...
            <div className="relative w-72 h-72 rounded-3xl overflow-hidden bg-black"
              ...
              {!scanResult && (<>
                {[['top-3 left-3','border-t-2 border-l-2 rounded-tl-xl'],['top-3 right-3','border-t-2 border-r-2 rounded-tr-xl'],
                  ...
                  <div key={i} className={`absolute ${pos} w-8 h-8 border-blue-400 ${cls}`} />
                ))}
