...
export async function initSimulation() {
  ...
  for (const lineId of linesToSimulate) {
    ...
    if (route.length > 0) {
      simulatedBuses.push({ id: `bus_${lineId}_1`, lineId, route, currentIndex: Math.floor(Math.random() * route.length), direction: 1, speedMultiplier: 1 });
      simulatedBuses.push({ id: `bus_${lineId}_2`, lineId, route, currentIndex: Math.floor(Math.random() * route.length), direction: -1, speedMultiplier: 2 });
    }
  }
  simInterval = window.setInterval(() => {
    const positions: (BusPosition & { busId: string })[] = simulatedBuses.map(bus => {
      ...
      return {
        ...
        lat: coord[0],
        lng: coord[1],
        speed: 40 * bus.speedMultiplier,
        occupancy: Math.floor(Math.random() * 100),
        driverId: driver?.id,
        timestamp: Date.now(),
      };
    });
    store.dispatch(setBusPositions(positions as any));
  }, 2000);
}
